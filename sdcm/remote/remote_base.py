...
class RemoteCmdRunnerBase(CommandRunner):  # pylint: disable=too-many-instance-attributes
    ...
    def run(self,
        ...
        :return:
        """
        print("rdebug Running remote_base.py run")
        try:
            print("rdebug remote_base running cmd of type", type(cmd))
            print("rdebug remote_base running cmd", cmd)
        except Exception as e:
            print("rdebug failed to print cmd")

        try:
            print("rdebug remote_base.py run _setup_watchers call log_file", log_file)
        except Exception as e:
            print("rdebug failed to print log_file in remote_base.py run()")
        watchers = self._setup_watchers(verbose=verbose, log_file=log_file, additional_watchers=watchers)
        try:
            print("rdebug remote_base.py run watchers", watchers)
            print("rdebug type watchers ", type(watchers))
        except Exception as e:
            print("rdebug failed to print watchers ", type(watchers))
